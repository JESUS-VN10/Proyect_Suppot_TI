...
class GestoresActas extends Controller
{
    ...
    public static function GestorSendMail(){
        $gestores = GestoreActas::where('copia_correo', true)->get();
    ...
    public static function DeleteGestorSendMail($id){
        $gestor = GestoreActas::find($id);
        if (!$gestor) {
            return response()->json(['success' => false, 'message' => 'Gestor no encontrado'], 404);
        }
        $gestor->copia_correo = false;
        $gestor->save();
        return response()->json(['success' => true, 'message' => 'Gestor actualizado correctamente']);
    }
