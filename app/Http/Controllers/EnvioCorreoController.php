...
class EnvioCorreoController extends Controller
{
    ...
    public function SendInventory(Request $request){
        ...
        // Llenar la lista con los usuarios que siempre tendrán la copia del acta
        $usuarios_copia = GestoresActas::GestorSendMail();

        // Asegúrate de que la respuesta tiene datos y la clave 'data'
        if (isset($usuarios_copia->original['data'])) {
            foreach ($usuarios_copia->original['data'] as $gestor_copia) {
