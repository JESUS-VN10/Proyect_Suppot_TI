      ...
      <form @submit.prevent="login">
        <div class="mb-4 relative">
          <span class="absolute inset-y-0 left-0 flex items-center pl-3">
            <i data-feather="credit-card" class="h-5 w-5 text-gray-800"></i>
          </span>
          <input type="text" v-model="datos_login.cedula" placeholder="Cédula" class="w-full pl-10 pr-3 py-2 bg-white bg-opacity-50 rounded-md text-gray-800 placeholder-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-600">
        </div>
        <div class="mb-6 relative">
          <span class="absolute inset-y-0 left-0 flex items-center pl-3">
            <i data-feather="lock" class="h-5 w-5 text-gray-800"></i>
          </span>
          <input type="password" v-model="datos_login.password" placeholder="Password" class="w-full pl-10 pr-3 py-2 bg-white bg-opacity-50 rounded-md text-gray-800 placeholder-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-600">
        </div>
      ...
      try {
        ...
        switch (res.data) {
          ...
          case 2:
            ...
            break;
          case 5:
            Swal.fire({title:'¡Ups!',text:'¡Este usuario esta bloqueado o inhabilitado, contacte al administrador!',icon:'info',toast:true,position: "top-end",timer:2000, showConfirmButton:false});
            break;
          case 4:
            this.contador += 1;
            if(this.contador >= 4){
