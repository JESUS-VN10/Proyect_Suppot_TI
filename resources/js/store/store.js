...
const store = new Vuex.Store({
    state: {
        ...
        usuario_session: [],
        lista_operaciones: [],
        // Nueva variable para guardar los directivos 'lista_directivos'
        lista_directivos: [],
        datos_form: [],
        name_gestor_session: '',
        inputs: 'outline-none ring-0 focus:ring-purple-500 transition duration-300 focus:ring-2 bg-opacity-80 text-slate-500 m-1 p-1',
        botones: 'bg-fuchsia-950 hover:bg-fuchsia-900 text-white font-bold py-2 px-4 rounded focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-opacity-50 m-2',
    ...
    mutations: {
        ...
        // Función para guardar formularios por parte del usuario
        saveForm(state, lista) {
            var documento = lista.documento;
            let datos = lista.form;
            let mapeo = new Map();
            try {
                // Validar si en el localStorage tiene datos y buscar y reemplazar los datos del usuario logueado.
                if (localStorage.length !== 0) {
                    ...
                    // Guardar datos si no encuentra al usuario en la cache crear una colección de datos
        ...
        // Función para cargar un acta guardada anteriormente por el usuario.
        ...
        // Generar la descarga con los archivos que están en el servidor
        ...
        // Nueva mutación para mostrar los directivos
        setDirectivos(state, directivos) {
            state.lista_directivos = directivos;
        },
        setOperaciones(state, operaciones) {
            state.lista_operaciones = operaciones;
        }
    },

    actions: {
        async cargarDirectivos({ commit }) {
            try {
                let response = await axios.get('/Actas_de_responsabilidad/Directivos');
                commit('setDirectivos', response.data);
            } catch (error) {
                console.error('Error al cargar directivos:', error);
            }
        },
        async cargarOperaciones({ commit }) {
            try {
                let response = await axios.get('/Actas_de_responsabilidad/Campanas');
                commit('setOperaciones', response.data);
            } catch (error) {
